...
// [dv/shim] Xcelium: xmvlog can't glue `__LINE__ into an identifier the way
// Verilator does, so the upstream typedef-based package static-assert fails
// to parse. Neutralized here — the width checks still run upstream under
// Verilator; this shim only affects the xrun build.
...
// [dv/shim] Xcelium: upstream UNUSED_VAR is Verilator lint plumbing and is
// sometimes invoked before the referenced net's declaration (use-before-decl
// error in xmvlog). Functionally a no-op -> neutralized for the xrun build.
`define UNUSED_VAR(x)
