...
// [dv/shim] NOTE: this compat copy is only valid for the 1-cluster config.
...
`endif
    ...
    for (genvar cluster_id = 0; cluster_id < `VX_CFG_NUM_CLUSTERS; ++cluster_id) begin : g_clusters
        ...
        `endif
            ...
            // [dv/shim] Xcelium: xmelab rejects +: part-selects of interface
            // instance arrays. Single-cluster config only (NUM_CLUSTERS==1),
            // so the slice IS the whole array — connect it directly.
            .mem_bus_if         (per_cluster_mem_bus_if),

            .kmu_bus_if         (per_cluster_kmu_bus_if), // [dv/shim] whole array (1 cluster)
